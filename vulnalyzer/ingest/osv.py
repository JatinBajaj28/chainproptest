...
def query_osv(package: str, ecosystem: str, version: str | None = None) -> list[dict]:
    ...
    headers = {
        ...
        "User-Agent": "Vulnalyzer/2.1.0 (https://github.com/vulnalyzer/vulnalyzer)"
    }
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count < max_retries:
                logger.warning("OSV timeout for %s/%s, retrying... (%d/%d)", 
                              ecosystem, package, retry_count, max_retries)
                time.sleep(2 ** retry_count)  # Exponential backoff
            else:
                logger.error("OSV query timeout after %d retries for %s/%s", 
                           max_retries, ecosystem, package)
        except requests.exceptions.HTTPError as exc:
            if exc.response.status_code == 429:  # Rate limited
                retry_count += 1
                if retry_count < max_retries:
                    logger.warning("OSV rate limited for %s/%s, retrying... (%d/%d)", 
                                  ecosystem, package, retry_count, max_retries)
                    time.sleep(5 * (2 ** retry_count))  # Longer backoff for rate limit
                else:
                    logger.error("OSV rate limited after %d retries for %s/%s", 
                               max_retries, ecosystem, package)
            else:
    ...
    while retry_count < max_retries:
        try:
            ...
            if resp.status_code == 200:
                ...
                retry_count += 1
                if retry_count < max_retries:
                    logger.warning("OSV rate limited for %s, retrying... (%d/%d)", 
                                  osv_id, retry_count, max_retries)
                    time.sleep(5 * (2 ** retry_count))
            else:
                logger.warning("OSV fetch failed for %s: HTTP %d", osv_id, resp.status_code)
                return None
        except requests.exceptions.Timeout:
            retry_count += 1
            if retry_count < max_retries:
                logger.warning("OSV timeout for %s, retrying... (%d/%d)", 
                              osv_id, retry_count, max_retries)
