    ...
    with get_conn() as conn:
        ...
        for scan in scans:
            ...
            for f in findings:
                ...
                # Also create a USES edge from the repo to the DIRECT dependency
                # (the first package in dependency_path).  For a transitive finding
                # like ts-node → mkdirp → minimist the vulnerable package is minimist
                # but ts-node is what the repo actually declares in its manifest.
                # Without this edge, "zipstream"-style intermediates are orphaned
                # from the repo node and the graph shows a shortcut A→C instead of
                # A→B→C.
                if len(dependency_path) >= 2:
                    direct_dep_name = dependency_path[0]
                    if direct_dep_name.lower() != pkg_name.lower():
                        direct_p_id = package_node_id(direct_dep_name, ecosystem)
                        upsert_graph_node(conn, direct_p_id, "Package", ecosystem, {
                            "name": direct_dep_name,
                            "ecosystem": ecosystem,
                        })
                        upsert_graph_edge(conn, r_id, direct_p_id, "USES", {
                            "direct": True,
                            "is_dev": is_dev,
                            "depth": 1,
                            "manifest": manifest,
