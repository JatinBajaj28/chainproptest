...
from vulnalyzer.scanner.github import parse_github_url, get_revision, fetch_file, get_repo_manifest_paths
...
def scan_repo(url: str, force: bool = False) -> ScanResult:
    ...
    # Discover ALL manifest files anywhere in the repo (not just root).
    # get_repo_manifest_paths uses the GitHub Trees API with recursive=1.
    all_filenames: set[str] = set(all_manifest_filenames())
    discovered_paths = get_repo_manifest_paths(owner, repo_name, revision_id, all_filenames)

    # Fall back to root-level filenames if the tree API returned nothing
    # (e.g. very large repos where GitHub truncates the tree response).
    if not discovered_paths:
        discovered_paths = list(all_manifest_filenames())

    for path in discovered_paths:
        basename = path.split("/")[-1]
        plugin = get_plugin_for_file(basename)
        if not plugin:
            continue

        text = fetch_file(owner, repo_name, revision_id, path)
        if text is None:
            continue
