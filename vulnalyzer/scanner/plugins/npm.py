    ...
    def parse(self, text: str):
        ...
        if "packages" in data:
            for pkg_path, meta in data["packages"].items():
                ...
                # Strip trailing slashes AND any trailing path separators that
                # appear on intermediate segments after splitting on "node_modules/".
                # e.g. "node_modules/express/node_modules/qs".split("node_modules/")
                # → ["", "express/", "qs"]
                # After filtering empty strings and stripping trailing "/":
                # → ["express", "qs"]
                # But we must also strip any trailing path component left after
                # splitting, e.g. "express/node_modules" → just "express".
                chain = []
                for x in rel:
                    if not x:
                        continue
                    # Remove trailing slash, then also any trailing /node_modules
                    part = x.rstrip("/")
                    # Split by "/" and take only the last meaningful component
                    # (removes leftover sub-paths from the split artifact)
                    subparts = part.split("/")
                    chain.append(subparts[0])
                pkg_name = chain[-1]
    ...
    def parse(self, text: str):
        ...
        for block in blocks:
            ...
            if a and b:
                ...
                # yarn.lock lists ALL packages (direct + transitive) without
                # nesting - cannot determine depth here, use 1 as safe default
                out[pkg] = DependencyInfo(
                    version=ver,
                    source="yarn.lock",
                    is_direct=False,
