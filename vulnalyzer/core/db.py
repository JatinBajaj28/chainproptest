    ...
    with get_conn() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS vulnerabilities (
                ...
                modified TEXT,
                raw_json TEXT,
                -- One row per (advisory, package, ecosystem) — an advisory
                -- can cover the same package in multiple ecosystems (npm, bower, …)
                -- Using osv_id alone as UNIQUE caused the last ecosystem to
                -- overwrite all prior ones, silently dropping npm findings.
                UNIQUE(osv_id, package_name, ecosystem)
            );
            ...
            CREATE TABLE IF NOT EXISTS scan_findings (
                ...
                -- osv_id intentionally not FK'd: vulnerabilities.osv_id
                -- is no longer unique (one row per osv_id+package+ecosystem).
            );
...
def upsert_vulnerability(
    ...
    *,
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(osv_id, package_name, ecosystem) DO UPDATE SET
...
def get_all_vulnerabilities(conn) -> list[dict]:
    ...
    for r in rows:
        ...
        # Filter by BOTH package name AND ecosystem to avoid merging ranges
        # from different ecosystems that share the same package name.
        affected_ranges: list[dict] = []
        for affected in raw.get("affected", []):
            pkg = affected.get("package", {})
            name_match = pkg.get("name", "").lower() == (r["package_name"] or "").lower()
            eco_match  = pkg.get("ecosystem", "").lower() == (r["ecosystem"] or "").lower()
            if not (name_match and eco_match):
                continue
